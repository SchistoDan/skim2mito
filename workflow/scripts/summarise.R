...
fastp_files <- list.files(file.path(results_dir, "fastp"), pattern = ".json", full.names = T)
# get read counts from json files
fastp_list <- lapply(fastp_files, function(x) {
  fastp_json <- fromJSON(x, flatten = T)
  sample_name <- gsub(paste0(file.path(results_dir, "fastp"), "/|_fastp.json"), "", x)
...
seqkit_files <- list.files(file.path(results_dir, "seqkit"), pattern = ".txt", full.names = T)
# read as list
...
seqkit_dat <- seqkit_dat %>%
  mutate(file = gsub(paste0(file.path(results_dir, "seqkit"), "/|\\.fasta"), "", file)) %>%
...
## blobtools
# get blobtools paths
blobtools_files <- list.files(file.path(results_dir, "blobtools"), recursive = T, pattern = "table.tsv", full.names = T)
# read as list
...
annotations_files <- list.files(file.path(results_dir, "annotations"), recursive = T, pattern = "result.bed", full.names = T)
# read as list
...
write.table(samples_out, file.path(results_dir, "summary/summary_samples_mqc.txt"), sep = "\t", col.names = T, row.names = F, quote = F)
write.table(contigs_out, file.path(results_dir, "summary/summary_contigs_mqc.txt"), sep = "\t", col.names = T, row.names = F, quote = F)
